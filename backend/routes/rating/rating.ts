...
export const rating = new Hono().use(
	jwt({
...
// create rating
rating.post("/rating.create", vValidator("json", ratingSchema), async (c) => {
...
rating.get("/rating.get/:id", async (c) => {
...
rating.get("/rating.getByUserId/:id", async (c) => {
	const id = c.req.param("id");
	console.log(id);
