...
export const carRequest = new Hono().use(
	jwt({
...
carRequest.post(
	"/car.request",
	vValidator("json", carRequestSchema),
	async (c) => {
...
carRequest.get("/car.request.list.by.branch/:branchId/:status", async (c) => {
	const user = c.get("jwtPayload");
...
carRequest.get("/car.request.list/:carId/:status", async (c) => {
	const user = c.get("jwtPayload");
...
// delete car request
carRequest.post("/car.request.delete/:carRequestId", async (c) => {
	const user = c.get("jwtPayload");
