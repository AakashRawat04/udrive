...
export const carJourney = new Hono().use(
	jwt({
...
carJourney.post(
	...
	async (c) => {
		...
		// start car journey
		const carJourneyStartResponse = await db.insert(carJourneyDbSchema).values({
			car: body.car,
			user: user.id,
...
carJourney.get("/car.journey.list.by.car/:carId", async (c) => {
...
carJourney.get("/car.journey.list.by.branch/:branchId", async (c) => {
...
carJourney.get("/car.journey.list.by.user/:userId", async (c) => {
...
// delete car journey
carJourney.delete("/car.journey.delete/:carJourneyId", async (c) => {
	const user = c.get("jwtPayload");
