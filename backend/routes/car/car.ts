...
car.post("/car.create", vValidator("json", carSchema), async (c) => {
...
car.put("/car.update", vValidator("json", updateCarSchema), async (c) => {
	const user = c.get("jwtPayload");
	const body = c.req.valid("json");
	console.log(body);
...
car.post("/car.assign", vValidator("json", assignCarSchema), async (c) => {
	const user = c.get("jwtPayload");
	const body = c.req.valid("json");
	console.log(body);
...
// get car by id
car.get("/car.getById/:id", async (c) => {
	const id = c.req.param("id");
	const car = await db.select().from(carDbSchema).where(eq(carDbSchema.id, id));
	return c.json({ data: car });
});

// get all cars
car.get("/car.getAll", async (c) => {
	const cars = await db.select().from(carDbSchema);
	return c.json({ data: cars });
});

// get all cars by branch id
car.get("/car.getByBranchId/:id", async (c) => {
	const id = c.req.param("id");
	const cars = await db
		.select()
		.from(carDbSchema)
...
car.delete("/car.delete/:id", async (c) => {
	const user = c.get("jwtPayload");
	const id = c.req.param("id");
	console.log(id);
